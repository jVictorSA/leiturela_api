...
def test_create_story():
    response = client.post("/stories/?story_prompt=Festa%20de%20halloween%20feliz", json={
    ...
    # Testar tentativa de criar uma história duplicada
    
    duplicate_story_prompt = "Festa de halloween feliz"  # Usar o mesmo prompt da história criada
    response_duplicate = client.post(f"/stories/?story_prompt={duplicate_story_prompt}", json={
        "base_story": {"história": "Base story example"},
        "sub_stories": {"sub_story_1": "Sub story example"}
    })
    
    print(f"Duplicate Story Status Code: {response_duplicate.status_code}")
    print(f"Duplicate Story Response Content: {response_duplicate.content}")

    # Verificar se a tentativa de criação de uma história duplicada retorna o código de erro apropriado
    
    assert response_duplicate.status_code == 400  
    assert response_duplicate.json() == {"detail": "História já existe no banco!!!"}
    # posso criar historia duplicada?
...
def test_delete_story():
    ...
    # Deletar a história
    response = client.delete("/stories/7")
    assert response.status_code == 204
