...
def test_register_user():
    response = client.post("/register", json={"email": "test10@example.com", "password": "securepassword"})
...
def test_login_user():
    ...
    client.post("/register", json={"email": "test10@example.com", "password": "securepassword"})

    # Tentar fazer login com credenciais corretas
    response = client.post("/login", json={"email": "test10@example.com", "password": "securepassword"})
    assert response.status_code == 200
    assert "access_token" in response.json()

    # Tentar fazer login com senha errada
    response = client.post("/login", json={"email": "test10@example.com", "password": "wrongpassword"})
    assert response.status_code == 400
